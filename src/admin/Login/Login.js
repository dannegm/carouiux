...
const LoginScreen = ({ requestLogin }) => {
    return (
        <LoginWrapper>
            ...
            <LoginDescription>
                Inicia sesión con tu cuenta de <BlueText>Google</BlueText> para acceder al{' '}
                <PurpleText>dashboard</PurpleText>.
            </LoginDescription>
            <LoginPrimaryButton variant='contained' size='large' onClick={() => requestLogin()}>
                Inicia con Google
            </LoginPrimaryButton>
        </LoginWrapper>
    );
...
const UnauthorizedScreen = ({ user, requestLogin, requestLogout }) => {
    return (
        <LoginWrapper>
            ...
            <LoginLinkButton onClick={() => requestLogout()}>Cerrar sesión</LoginLinkButton>
        </LoginWrapper>
    );
};
...
const Login = () => {
    ...
    return !hasSession ? (
        ...
        <UnauthorizedScreen requestLogin={requestLogin} requestLogout={requestLogout} user={user} />
    );
};
