...
const EditProject = () => {
    ...
    const { uid } = useParams();
    const [originalProjectModel, setOriginalProjectModel] = useState(INITIAL_PROJECT);
    const [projectModel, setProjectModel] = useState(INITIAL_PROJECT);
    const [isLoading, setLoading] = useState(true);

    const fetchProject = async (projectID) => {
        const projectDoc = doc(db, dbCollections.PROJECTS, projectID);
        const projectSnapshop = await getDoc(projectDoc);
    ...
    const validations = {
        ...
        callToActionLabel: validator(projectModel.callToActionLabel, [noEmptyString()]),
    };
    ...
    const handleSave = async () => {
        if (isValid) {
            try {
                const projectDoc = doc(db, dbCollections.PROJECTS, projectModel.uid);
                await setDoc(projectDoc, projectModel);
